...
from Design import config, const
import telebot
...
class Item:
    ...
    def get_desc2(self):
        ...
        btn_name = telebot.types.InlineKeyboardButton(text=data[0], callback_data='.')
        btn_price = telebot.types.InlineKeyboardButton(text=str(data[2]) + " RUB", callback_data='.' + str(self.id))
        btn_buy = telebot.types.InlineKeyboardButton(text="Купить", callback_data=str(self.id))
        markup.row(btn_name)
